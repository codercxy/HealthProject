...
public class Chart_4_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_chart_4_);

        chart = (ColumnChartView) findViewById(R.id.chart_column);
    ...
    private void generateSubcolumnsData() {
        ...
        for (int i = 0; i < numColumns; ++i) {
            ...
            values = new ArrayList<SubcolumnValue>();
            for (int j = 0; j < numSubcolumns; ++j) {
                values.add(new SubcolumnValue((float) Math.random() * 50f + 5, ChartUtils.COLORS[j % numSubcolumns]));
            }
        ...
        if (hasAxes) {
            ...
            Axis axisY = new Axis().setHasLines(true);
            if (hasAxesNames) {
                axisX.setName("周末头痛/怪梦/口腔疼痛/牙龈出血");
                axisY.setName("");
            }
            data.setAxisXBottom(axisX);
            data.setAxisYLeft(axisY);

...
import com.nju.android.health.model.data.Weather;
 ...
 * Time: 下午2:39
 */
public class CircleBarTiny extends View {
	...
	public CircleBarTiny(Context context) {
		super(context);
		init();
	}

	public CircleBarTiny(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public CircleBarTiny(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init() {
		...
		circleStrokeWidth = DisplayUtil.dip2px(getContext(), 10);// 圆圈的线条粗细
		mTextSize = DisplayUtil.sp2px(getContext(), 40);// 中间文字大小
		mTextSize2 = DisplayUtil.sp2px(getContext(), 25);
		mTextSize1 = DisplayUtil.sp2px(getContext(), 12);// 上下文字大小
		mDistance = DisplayUtil.dip2px(getContext(), 30);// 文字间的距离
		mDistance1 = DisplayUtil.dip2px(getContext(), 10);//圆圈离父布局的距离
		...
		mDefaultWheelPaint.setAlpha(100);
